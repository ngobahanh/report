...
export default function TrainingReports() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const response = await trainingReportsAPI.getAll(page);
  ...
  const handleDelete = async (id) => {
    if (confirm('Bạn chắc chắn muốn xóa?')) {
      try {
        await trainingReportsAPI.delete(id);
  ...
  const handleSubmit = async () => {
    try {
      if (editingId) {
        await trainingReportsAPI.update(editingId, formData);
        alert('Cập nhật thành công');
      } else {
        await trainingReportsAPI.create(formData);
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Báo cáo huấn luyện</h1>
          <p className="text-gray-600 mt-1">Quản lý báo cáo huấn luyện</p>
        </div>
        <button
          onClick={handleAdd}
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-sm font-semibold text-gray-700">Số giờ</th>
                    <th className="px-6 py-3 text-left text-sm font-semibold text-gray-700">Người huấn luyện</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {data.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600">{item.training_hours}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{item.trainer?.full_name}</td>
      ...
      >
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Nội dung huấn luyện</label>
            <textarea
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Vấn đề huấn luyện</label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Số giờ</label>
              <input
                type="number"
                step="0.5"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Người huấn luyện</label>
            <select
